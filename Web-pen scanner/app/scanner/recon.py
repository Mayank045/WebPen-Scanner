...
def _request_target(url, timeout=10, redirect_validator=None):
    current_url = url
    ...
    for _ in range(MAX_REDIRECTS + 1):
        ...
        if response.is_redirect:
            ...
            if redirect_validator and not redirect_validator(next_url):
                raise ReconError("The target redirect left the allowed scope.")
        ...
        try:
            ...
            raise ReconError("The target response could not be read.") from error
        finally:
            response.close()
